...
export default async function generateTVSitemap() {
  await tvSitemap();
}
export const tvSitemap = async () => {
  return;
  ...
  const Pages = myArray.map((i) =>
    tmdb.discoverTv([{ param: "page", value: i }])
  );
  ...
  fs.writeFileSync("./public/sitemap_tvshows5.xml", rssOuterFormat(items));
};
...
const itemFormatter = (item) => {
  ...
  <url>
    <loc>https://www.hiptoro.com/tv-shows/${generateMovieSlug({
      title: item.name,
      id: item.id,
    })}</loc>
    <lastmod>2023-06-27T05:19:53.704Z</lastmod>
