...
export default function InfiniteAnimeGrid(props: PostgridProps) {
  ...
  const getMoreMovies = async () => {
    if (posts.length < 100) {
      ...
      const response = await fetch(`/api/anime?total=${pageNoToGet}`).then(
        (r) => r.json()
      );
        ...
        {posts.map((post) => (
          ...
          >
            ...
            <div className="md:px-2 flex flex-col items-start justify-center">
              <Link className="hover:text-red-400" href={`/anime/${post.slug}`}>
                <p
